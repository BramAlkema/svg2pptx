...
from .visual_report_service import VisualReportService
from ..converters.marker_processor import MarkerProcessor
from ..color.profile_registry import ColorProfileRegistry, ProfileRegistryConfig
from ..color.profile_asset_loader import ColorProfileAssetLoader, AssetLoadingPolicy
...
class ConversionConfig:
    ...
    # ICC Color Profile System Configuration
    enable_color_profiles: bool = True
    profile_cache_directory: Optional[str] = None
    allow_remote_profiles: bool = False
    allow_data_url_profiles: bool = True
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'enable_color_profiles': self.enable_color_profiles,
            'profile_cache_directory': self.profile_cache_directory,
            'allow_remote_profiles': self.allow_remote_profiles,
            'allow_data_url_profiles': self.allow_data_url_profiles,
            'max_profile_file_size': self.max_profile_file_size
        }
...
class ConversionServices:
    ...
    visual_report_service: VisualReportService
    config: ConversionConfig = None

    # ICC Color Profile System
    color_profile_registry: Optional[ColorProfileRegistry] = None
    color_profile_asset_loader: Optional[ColorProfileAssetLoader] = None
    ...
    def create_default(cls, config: Optional[ConversionConfig] = None,
        ...
        try:
            ...
            # Initialize ICC Color Profile System if enabled
            color_profile_registry = None
            color_profile_asset_loader = None
            if config.enable_color_profiles:
                try:
                    # Create asset loading policy from config
                    asset_policy = AssetLoadingPolicy(
                        allow_remote_loading=config.allow_remote_profiles,
                        allow_data_urls=config.allow_data_url_profiles,
                        max_file_size=config.max_profile_file_size
                    )

                    # Create asset loader with policy and cache directory
                    cache_dir = Path(config.profile_cache_directory) if config.profile_cache_directory else None
                    color_profile_asset_loader = ColorProfileAssetLoader(
                        policy=asset_policy,
                        cache_directory=cache_dir
                    )

                    # Create profile registry with asset loader integration
                    registry_config = ProfileRegistryConfig(
                        enable_remote_profiles=config.allow_remote_profiles,
                        enable_data_urls=config.allow_data_url_profiles
                    )
                    color_profile_registry = ColorProfileRegistry(
                        config=registry_config,
                        asset_loader=color_profile_asset_loader
                    )

                    logger.info("ICC Color Profile System initialized")
                except Exception as e:
                    logger.warning(f"Failed to initialize color profile system: {e}")
                    color_profile_registry = None
                    color_profile_asset_loader = None
            ...
            return cls(
                ...
                visual_report_service=visual_report_service,
                config=config,
                color_profile_registry=color_profile_registry,
                color_profile_asset_loader=color_profile_asset_loader
            )
    ...
    def create_custom(cls, custom_config: Dict[str, Dict[str, Any]]) -> 'ConversionServices':
        ...
        try:
            ...
            # Initialize ICC Color Profile System if enabled (same as create_default)
            color_profile_registry = None
            color_profile_asset_loader = None
            if config.enable_color_profiles:
                try:
                    ...
                    logger.info("ICC Color Profile System initialized (custom)")
                except Exception as e:
                    logger.warning(f"Failed to initialize color profile system: {e}")
                    color_profile_registry = None
                    color_profile_asset_loader = None
    ...
    def cleanup(self) -> None:
        ...
        # Clean up ICC Color Profile System
        if self.color_profile_asset_loader:
            self.color_profile_asset_loader.clear_cache()
        self.color_profile_registry = None
        self.color_profile_asset_loader = None
    ...
    def validate_services(self) -> bool:
        ...
        try:
            ...
            self.viewport_resolver.parse_viewbox_strings(["0 0 100 100"])
            self.viewport_resolver.svg_to_emu(10, 20)
            ...
            self.image_service.process_image_source("/nonexistent/path")  # Should return None gracefully

            # Test ICC Color Profile System if enabled
            if self.color_profile_registry:
                profiles = self.color_profile_registry.list_profiles()  # Should return built-in profiles
                assert len(profiles) >= 5, "Built-in profiles should be available"
                assert "srgb" in profiles, "sRGB profile should be built-in"

                # Test profile resolution
                srgb_profile = self.color_profile_registry.resolve_profile("srgb")
                assert srgb_profile is not None, "sRGB profile should resolve"

                # Test registry stats
                stats = self.color_profile_registry.get_registry_stats()
                assert isinstance(stats, dict), "Registry stats should be a dictionary"

            if self.color_profile_asset_loader:
                # Test cache stats
                cache_stats = self.color_profile_asset_loader.get_cache_stats()
                assert isinstance(cache_stats, dict), "Asset loader cache stats should be a dictionary"
