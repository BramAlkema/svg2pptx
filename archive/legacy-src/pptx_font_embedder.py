...
class PPTXFontEmbedder:
    ...
    def _embed_fonts_via_zip_manipulation(self, presentation: Presentation, temp_dir: Path = None) -> Presentation:
        ...
        if temp_dir is None:
            secure_dir = default_secure_file_service.create_secure_temp_dir(prefix="font_embed_")
            temp_dir = Path(secure_dir.path)
...
class PPTXTextWithEmbeddedFonts:
    ...
    def process_svg_text_element(self, text_element: ET.Element) -> str:
        ...
        font_size_str = text_element.get('font-size', '12')
        parsed_size = self._validator.parse_length_safe(font_size_str, default_unit='px')
        size = int(parsed_size) if parsed_size is not None else 12
