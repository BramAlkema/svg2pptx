#!/usr/bin/env python3
"""
Shape Geometry Engine for SVG2PPTX.

Provides vectorized shape generators with performance improvements
over the legacy scalar implementation. Handles batch processing of circles,
ellipses, rectangles, polygons, and lines with efficient operations.
...
class GeometryEngine:
    """
    High-performance geometry engine for SVG shapes.
...
def create_geometry_engine(optimization_level: int = 2) -> GeometryEngine:
    """Create a geometry engine with specified optimization level."""
    return GeometryEngine(optimization_level)

def batch_process_shapes(shape_data: Dict[str, Any],
                        engine: Optional[GeometryEngine] = None) -> List[ShapeGeometry]:
    """
