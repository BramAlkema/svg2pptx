#!/usr/bin/env python3
"""
NumPy-accelerated Shape Converter for SVG2PPTX.

Integrates the high-performance NumPy geometry engine with the existing
converter infrastructure to provide 25-70x performance improvements while
...
class NumPyShapeConverter(BaseConverter):
    """
    High-performance shape converter using NumPy vectorization.
    ...
    def _convert_circle(self, element: ET.Element, context: ConversionContext) -> str:
        """Convert single circle using vectorized operations."""
    ...
    def _convert_rectangles_batch(self, elements_info: List[Dict], context: ConversionContext) -> List[str]:
        ...
        for i, info in enumerate(elements_info):
            ...
            positions[i] = [
                self.parse_length(element.get('x', '0')),
                self.parse_length(element.get('y', '0'))
            ]
            dimensions[i] = [
    ...
    def _convert_lines_batch(self, elements_info: List[Dict], context: ConversionContext) -> List[str]:
        ...
        for i, info in enumerate(elements_info):
            element = info['element']
            start_points[i] = [
                self.parse_length(element.get('x1', '0')),
                self.parse_length(element.get('y1', '0'))
            ]
            end_points[i] = [
                self.parse_length(element.get('x2', '0')),
                self.parse_length(element.get('y2', '0'))
            ]
    ...
    def _generate_circle_drawingml(self, element: ET.Element, geometry: ShapeGeometry,
                                 context: ConversionContext, index: int) -> str:
        """Generate DrawingML for circle using vectorized geometry data."""
        # Convert to EMU coordinates
        emu_x, emu_y = self._convert_svg_to_drawingml_coords(
            geometry.positions[index][0], geometry.positions[index][1], context
        )
        diameter = geometry.dimensions[index][0]  # Both width and height are diameter
    ...
    def _generate_line_drawingml(self, element: ET.Element, geometry: ShapeGeometry,
                               context: ConversionContext, index: int) -> str:
        """Generate DrawingML for line using vectorized geometry data."""
        # Convert to EMU coordinates
        emu_x, emu_y = self._convert_svg_to_drawingml_coords(
            geometry.positions[index][0], geometry.positions[index][1], context
        )
        width, height = geometry.dimensions[index]
        emu_width = context.coordinate_system.svg_length_to_emu(max(width, 1), 'x')
        emu_height = context.coordinate_system.svg_length_to_emu(max(height, 1), 'y')

        # Get line direction from parameters
        direction = geometry.parameters[index]  # [dx, dy] direction vector

        # Calculate path coordinates based on direction
        if direction[0] >= 0 and direction[1] >= 0:
            start_x, start_y, end_x, end_y = 0, 0, 21600, 21600
        elif direction[0] < 0 and direction[1] >= 0:
            start_x, start_y, end_x, end_y = 21600, 0, 0, 21600
        elif direction[0] >= 0 and direction[1] < 0:
            start_x, start_y, end_x, end_y = 0, 21600, 21600, 0
        else:
            start_x, start_y, end_x, end_y = 21600, 21600, 0, 0
