#!/usr/bin/env python3
"""
Core Viewport & Aspect Ratio Engine for SVG2PPTX

High-performance viewport resolution with vectorized operations.
...
Key Features:
- Vectorized viewport operations
...
class ViewportEngine:
    ...
    def __init__(self, unit_engine: UnitConverter | None = None):
        ...
        """
        self.unit_engine = unit_engine or UnitConverter()
        ...
        # Fluent API state
        self._svg_elements = None
        self._target_sizes = None
        self._alignment = AspectAlign.X_MID_Y_MID
        self._meet_or_slice = MeetOrSlice.MEET
        self._contexts = None
    ...
    def for_svg(self, svg_element: ET.Element) -> 'ViewportEngine':
        """Set SVG element for processing. Supports method chaining."""
    ...
    def with_slide_size(self, width: int, height: int) -> 'ViewportEngine':
        """Set target slide dimensions in EMU. Supports method chaining."""
    ...
    def center(self) -> 'ViewportEngine':
        """Center content in viewport. Supports method chaining."""
        self._alignment = AspectAlign.X_MID_Y_MID
        return self

    def top_left(self) -> 'ViewportEngine':
        """Align content to top-left. Supports method chaining."""
        self._alignment = AspectAlign.X_MIN_Y_MIN
        return self

    def top_right(self) -> 'ViewportEngine':
        """Align content to top-right. Supports method chaining."""
        self._alignment = AspectAlign.X_MAX_Y_MIN
        return self

    def bottom_left(self) -> 'ViewportEngine':
        """Align content to bottom-left. Supports method chaining."""
        self._alignment = AspectAlign.X_MIN_Y_MAX
        return self

    def bottom_right(self) -> 'ViewportEngine':
        """Align content to bottom-right. Supports method chaining."""
        self._alignment = AspectAlign.X_MAX_Y_MAX
        return self

    def meet(self) -> 'ViewportEngine':
        """Scale to fit entirely within viewport (preserves aspect ratio). Supports method chaining."""
        self._meet_or_slice = MeetOrSlice.MEET
        return self

    def slice(self) -> 'ViewportEngine':
        """Scale to fill entire viewport (may crop content). Supports method chaining."""
        self._meet_or_slice = MeetOrSlice.SLICE
        return self

    def with_context(self, context) -> 'ViewportEngine':
        """Set conversion context. Supports method chaining."""
    ...
    def resolve(self) -> np.ndarray:
        """Execute viewport resolution and return structured array of mappings."""
        if self._svg_elements is None:
            raise ValueError("SVG element(s) must be set using for_svg() or for_svgs()")
    ...
    def resolve_single(self):
        """Execute viewport resolution for single SVG and return first mapping."""
        mappings = self.resolve()
        return mappings[0] if len(mappings) > 0 else None
    ...
    def extract_viewport_dimensions_batch(self, svg_elements: list[ET.Element],
        ...
        width_results = self.unit_engine.batch_convert_dict(width_dict, contexts[0] if contexts else None)
        height_results = self.unit_engine.batch_convert_dict(height_dict, contexts[0] if contexts else None)
    ...
    def calculate_viewport_mappings_batch(self,
        ...
        if np.any(preserve_mask):
            ...
            # Fix: Use proper structured array assignment
            # Direct field assignment works correctly with advanced indexing
            for i, idx in enumerate(preserve_subset):
                result[idx]['scale_x'] = uniform_scales[i]
                result[idx]['scale_y'] = uniform_scales[i]
                result[idx]['translate_x'] = -preserve_viewboxes['min_x'][i] * uniform_scales[i] + offset_x[i]
                result[idx]['translate_y'] = -preserve_viewboxes['min_y'][i] * uniform_scales[i] + offset_y[i]
                result[idx]['content_width'] = int(scaled_widths[i])
                result[idx]['content_height'] = int(scaled_heights[i])
            ...
            # Fix: Assign clipping needs individually
            for i, idx in enumerate(preserve_subset):
                result[idx]['clip_needed'] = bool(clip_needed[i])

        # Fill viewport dimensions for all valid cases
        # Fix: Use proper structured array assignment for viewport dimensions
        valid_indices = np.where(valid_mask)[0]
        for i, idx in enumerate(valid_indices):
            result[idx]['viewport_width'] = valid_viewports['width'][i]
            result[idx]['viewport_height'] = valid_viewports['height'][i]
    ...
    def batch_resolve_svg_viewports(self, svg_elements: list[ET.Element],
        ...
        # Calculate actual content bounds instead of just using viewBox
        viewboxes = np.zeros(n_elements, dtype=ViewBoxArray)

        for i, svg in enumerate(svg_elements):
            try:
                # Calculate actual content bounding box after transforms
                ...
                if width > 0 and height > 0:
                    ...
                    if len(parts) == 4:
                        ...
                        aspect_ratio = vb_w / vb_h if vb_h > 0 else 1.0
                        viewboxes[i] = (vb_x, vb_y, vb_w, vb_h, aspect_ratio)
                    else:
                        viewboxes[i] = (0, 0, 100, 100, 1.0)
            except Exception:
                # Fallback to viewBox on any error
                viewbox_str = svg.get('viewBox', '0 0 100 100')
                parts = viewbox_str.split()
                if len(parts) == 4:
                    vb_x, vb_y, vb_w, vb_h = [float(p) for p in parts]
        ...
        # Calculate complete mappings and return structured array directly
    ...
    def calculate_shape_bounding_box_and_relative_coords(self, svg_coords, viewport_mapping):
        ...
        This method provides centralized coordinate transformation for all shape converters,
        ensuring consistent handling across paths, rectangles, circles, polygons, and filters.

        Args:
            svg_coords: List of (x, y) tuples in SVG coordinate space
            viewport_mapping: Viewport mapping structured array from resolve_single()
        ...
        # Extract transformation parameters from viewport mapping
        ...
        # Convert bounding box to EMU coordinates using viewport transformation
        ...
        # Convert all coordinates to relative positions (0-100000 range for DrawingML)
        relative_coords = []
    ...
    def svg_to_emu(self, x: float, y: float) -> tuple[int, int]:
        ...
        Args:
            x: SVG x coordinate (in pixels or other SVG units)
            y: SVG y coordinate (in pixels or other SVG units)

        Returns:
            Tuple of (x_emu, y_emu) in EMU units
        """
        # Use unit_engine to convert to EMU, assuming pixel units by default
        x_emu = self.unit_engine.to_emu(f"{x}px")
    ...
    def svg_to_emu_with_units(self, x: str, y: str) -> tuple[int, int]:
        ...
        Args:
            x: SVG x coordinate with units (e.g., "10px", "2cm", "1in")
            y: SVG y coordinate with units (e.g., "10px", "2cm", "1in")

        Returns:
            Tuple of (x_emu, y_emu) in EMU units
        """
        x_emu = self.unit_engine.to_emu(x)
    ...
    def svg_to_emu_batch(self, coordinates: np.ndarray | list[tuple[float, float]]) -> np.ndarray:
        ...
        Args:
            coordinates: Array or list of (x, y) coordinate pairs

        Returns:
            NumPy array of (x_emu, y_emu) pairs in EMU units
        """
        ...
        # Convert to EMU using batch operations
        # Assuming px units by default, convert to EMU (1px = 12700 EMU)
        x_emu = (x_coords * 12700).astype(int)
        y_emu = (y_coords * 12700).astype(int)

        return np.column_stack((x_emu, y_emu))
