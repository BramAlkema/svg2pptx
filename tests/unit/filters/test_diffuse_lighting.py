#!/usr/bin/env python3
"""
Unit tests for DiffuseLightingProcessor.

This test suite focuses on testing the SVG feDiffuseLighting filter processor,
including 3D lighting effects using PowerPoint sp3d, bevel, and lightRig features.
"""
...
class TestDiffuseLightingParameters:
    """Test DiffuseLightingParameters class."""
    ...
    def test_custom_initialization(self):
        ...
        params = DiffuseLightingParameters(
            ...
            light_z=75.0,
            input_source="blur1",
            result_name="diffuse_light"
        )

        assert params.surface_scale == 3.0
        assert params.diffuse_constant == 2.0
        ...
        assert params.result_name == "diffuse_light"

    def test_post_init_validation(self):
        """Test parameter validation in __post_init__."""
        # Test surface scale clamping
        params = DiffuseLightingParameters(surface_scale=100.0)
        assert params.surface_scale == 50.0  # Clamped to max

        params = DiffuseLightingParameters(surface_scale=-5.0)
        assert params.surface_scale == 0.0  # Clamped to min

        # Test diffuse constant clamping
        params = DiffuseLightingParameters(diffuse_constant=20.0)
        assert params.diffuse_constant == 10.0  # Clamped to max

        # Test angle normalization
        params = DiffuseLightingParameters(light_azimuth=450.0)
        assert params.light_azimuth == 90.0  # 450 % 360

        params = DiffuseLightingParameters(light_elevation=100.0)
        assert params.light_elevation == 90.0  # Clamped to max

        # Test invalid light source type
        params = DiffuseLightingParameters(light_source_type="invalid")
        assert params.light_source_type == "distant"  # Default

    def test_complexity_score_calculation(self):
        """Test complexity score calculation."""
        # Simple case
        simple_params = DiffuseLightingParameters(
            surface_scale=1.0,
        ...
        # Complex case
        complex_params = DiffuseLightingParameters(
            surface_scale=10.0,
            diffuse_constant=5.0,
            lighting_color="#FF6600",
    ...
    def test_is_effective(self):
        ...
        # Effective lighting
        effective_params = DiffuseLightingParameters(
            surface_scale=2.0,
            diffuse_constant=1.0
        )
        ...
        ineffective_params = DiffuseLightingParameters(
            surface_scale=0.0,
    ...
    def test_get_powerpoint_light_direction(self):
        """Test PowerPoint light direction mapping."""
        # Top lighting
        top_params = DiffuseLightingParameters(
            light_source_type="distant",
            light_azimuth=0.0,
            light_elevation=90.0
        )
        assert top_params.get_powerpoint_light_direction() == "t"

        # Side lighting
        side_params = DiffuseLightingParameters(
            light_source_type="distant",
            light_azimuth=90.0,
            light_elevation=0.0
        )
        direction = side_params.get_powerpoint_light_direction()
        assert direction in ["br", "tr", "bl", "tl"]


class TestDiffuseLightingProcessor:
    """Test DiffuseLightingProcessor class."""

    def setup_method(self):
        """Setup test fixtures."""
        self.processor = DiffuseLightingProcessor()
    ...
    def test_initialization(self):
        ...
        processor = DiffuseLightingProcessor()
        assert processor.filter_type == 'feDiffuseLighting'
        assert processor.max_surface_scale == 10.0
        assert processor.max_extrusion_emu == 1270000

    def test_factory_function(self):
        """Test factory function."""
        processor = create_diffuse_lighting_processor()
        assert isinstance(processor, DiffuseLightingProcessor)
        assert processor.filter_type == 'feDiffuseLighting'

    def test_can_apply_valid_element(self):
        """Test can_apply with valid feDiffuseLighting element."""
        element = ET.fromstring("""
            <feDiffuseLighting surfaceScale="2.0" diffuseConstant="1.0">
    ...
    def test_parse_distant_light_parameters(self):
        ...
        element = ET.fromstring("""
            <feDiffuseLighting surfaceScale="3.0" diffuseConstant="2.0" lighting-color="#FF6600">
                <feDistantLight azimuth="45" elevation="60"/>
            </feDiffuseLighting>
        """)

        params = self.processor._parse_diffuse_lighting_parameters(element)
    ...
    def test_parse_point_light_parameters(self):
        ...
        element = ET.fromstring("""
            <feDiffuseLighting surfaceScale="2.0" diffuseConstant="1.5">
    ...
    def test_parse_spot_light_parameters(self):
        ...
        element = ET.fromstring("""
            <feDiffuseLighting surfaceScale="1.5" diffuseConstant="1.0">
    ...
    def test_can_use_native_3d(self):
        ...
        complex_params = DiffuseLightingParameters(
            surface_scale=20.0,  # Too high
            diffuse_constant=8.0  # Too intense
        )
    ...
    def test_apply_ineffective_lighting(self):
        ...
        element = ET.fromstring("""
            <feDiffuseLighting surfaceScale="0.0" diffuseConstant="0.0">
        ...
        assert result.success
        assert result.strategy == FilterStrategy.NATIVE
        assert "No visible diffuse lighting effect" in result.drawingml
    ...
    def test_generate_sp3d_configuration(self):
        ...
        assert "surface_scale=2.0" in drawingml
        assert "diffuse_constant=1.5" in drawingml
    ...
    def test_generate_inner_shadow_depth(self):
        """Test inner shadow generation for depth perception."""
        params = DiffuseLightingParameters(
            surface_scale=2.0,
            diffuse_constant=1.0,
            light_source_type="distant",
        ...
        drawingml = self.processor._generate_inner_shadow_depth(params, self.mock_context)

        assert "a:innerShdw" in drawingml
        assert "blurRad=" in drawingml
        assert "kx=" in drawingml
        assert "ky=" in drawingml
    ...
    def test_apply_approximation_strategy(self):
        ...
        with patch.object(self.processor, '_get_strategy', return_value=FilterStrategy.APPROXIMATION):
            element = ET.fromstring("""
                <feDiffuseLighting surfaceScale="15.0" diffuseConstant="8.0">
                    <feDistantLight azimuth="45" elevation="60"/>
                </feDiffuseLighting>
            """)
    ...
    def test_apply_with_exception(self):
        ...
        invalid_element = ET.fromstring('<feDiffuseLighting surfaceScale="invalid"/>')
    ...
    def test_point_light_3d_effects(self):
        ...
        element = ET.fromstring("""
            <feDiffuseLighting surfaceScale="3.0" diffuseConstant="2.0">
    ...
    def test_spot_light_3d_effects(self):
        ...
        assert result.success
        assert "spot" in result.drawingml.lower()

    def test_colored_lighting_effects(self):
        """Test colored lighting coordination."""
        element = ET.fromstring("""
            <feDiffuseLighting surfaceScale="2.0" diffuseConstant="1.0" lighting-color="#FF6600">
    ...
    def test_edge_case_minimal_parameters(self):
        ...
        element = ET.fromstring("""
            <feDiffuseLighting surfaceScale="0.1" diffuseConstant="0.1">
                <feDistantLight/>
            </feDiffuseLighting>
        """)
    ...
    def test_edge_case_maximum_parameters(self):
        ...
        element = ET.fromstring("""
            <feDiffuseLighting surfaceScale="50.0" diffuseConstant="10.0">
...
class TestDiffuseLightingIntegration:
    """Integration tests for diffuse lighting processor."""
    ...
    def test_complete_3d_effect_integration(self):
        ...
        element = ET.fromstring("""
            <feDiffuseLighting surfaceScale="3.0" diffuseConstant="1.5" lighting-color="#FFFFE0">
    ...
    def test_high_intensity_lighting_combination(self):
        ...
        assert result.success
        # This intensity should work with native strategy
        assert result.strategy == FilterStrategy.NATIVE

        # Verify the result contains proper 3D effects
        assert "a:sp3d" in result.drawingml
        assert "extrusionH" in result.drawingml
    ...
    def test_metadata_completeness(self):
        ...
        element = ET.fromstring("""
            <feDiffuseLighting surfaceScale="2.0" diffuseConstant="1.0" lighting-color="#FF6600" result="diffuse_result">
                <feDistantLight azimuth="45" elevation="60"/>
            </feDiffuseLighting>
        """)
        ...
        assert result.success
        assert result.metadata['filter_type'] == 'feDiffuseLighting'
        assert result.metadata['approach'] == 'native_3d'
        assert result.metadata['surface_scale'] == 2.0
        assert result.metadata['diffuse_constant'] == 1.0
        assert result.metadata['lighting_color'] == "#FF6600"
