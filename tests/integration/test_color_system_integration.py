...
from src.colors import (
    ColorParser, ColorInfo, ColorFormat,
    calculate_delta_e_cie76, calculate_luminance, calculate_contrast_ratio,
    clamp_rgb, normalize_color, simulate_colorblindness
)
...
@pytest.fixture
def color_parser():
    """Create ColorParser for integration testing."""
    return ColorParser()
...
class TestGradientConversionIntegration:
    ...
    def test_complex_linear_gradient_processing(self, color_parser, gradient_converter, mock_context):
        ...
        for stop in stops:
            ...
            if 'stop-color:' in color_style:
                ...
                parsed_color = color_parser.parse(color_str)
                assert parsed_color.format in [ColorFormat.RGB, ColorFormat.RGBA, ColorFormat.HEX, ColorFormat.HSL, ColorFormat.HSLA, ColorFormat.NAMED]
    ...
    def test_radial_gradient_with_transforms(self, color_parser, gradient_converter, mock_context):
        ...
        for i in range(len(stops) - 1):
            ...
            current_color = color_parser.parse(current_stop.get('style', '').split('stop-color:')[1].strip())
            next_color = color_parser.parse(next_stop.get('style', '').split('stop-color:')[1].strip())
    ...
    def test_gradient_with_css_named_colors(self, color_parser, gradient_converter, mock_context):
        ...
        for stop, expected_name in zip(stops, expected_colors):
            ...
            parsed_color = color_parser.parse(color_str)
            assert parsed_color.format == ColorFormat.NAMED
            assert parsed_color.original_value == expected_name
    ...
    def test_gradient_color_space_accuracy(self, color_parser, gradient_converter, mock_context):
        ...
        red_color = color_parser.parse('#FF0000')
        assert red_color.rgb_tuple == (255, 0, 0)
        assert red_color.alpha == 1.0

        # Mid gray should have equal RGB components
        gray_color = color_parser.parse('#808080')
        assert gray_color.rgb_tuple == (128, 128, 128)

        # Pure blue should have specific RGB values
        blue_color = color_parser.parse('#0000FF')
        assert blue_color.rgb_tuple == (0, 0, 255)
...
class TestPerformanceBenchmarking:
    ...
    def test_color_parsing_performance_benchmark(self, color_parser):
        ...
        for color_str in test_colors:
            try:
                parsed_colors.append(color_parser.parse(color_str))
            except Exception:
                continue
        ...
        for r in range(0, 256, 32):
            for g in range(0, 256, 32):
                for b in range(0, 256, 32):
                    test_colors.append(color_parser.parse(f'rgb({r}, {g}, {b})'))

        # Benchmark RGB to LAB conversion
        start_time = time.time()
        lab_conversions = []
        for color in test_colors:
            lab_color = color.to_lab()
    ...
    def test_memory_usage_benchmark(self, color_parser):
        ...
        for i in range(10000):
            ...
            colors.append(color_parser.parse(f'rgb({r}, {g}, {b})'))
...
class TestColorAccuracyValidation:
    ...
    def test_srgb_color_space_accuracy(self, color_parser):
        ...
        for hex_color, expected_rgb in reference_colors:
            parsed_color = color_parser.parse(hex_color)
            assert parsed_color.rgb_tuple == expected_rgb, f"Color {hex_color} parsed incorrectly"
    ...
    def test_lab_color_space_accuracy(self, color_parser):
        ...
        for rgb, expected_lab in test_cases:
            color = color_parser.parse(f'rgb({rgb[0]}, {rgb[1]}, {rgb[2]})')
    ...
    def test_delta_e_calculation_accuracy(self, color_parser):
        ...
        for (color1_hex, color2_hex), expected_delta_e in test_pairs:
            color1 = color_parser.parse(color1_hex)
            color2 = color_parser.parse(color2_hex)
    ...
    def test_luminance_calculation_accuracy(self, color_parser):
        ...
        for hex_color, expected_luminance in test_cases:
            color = color_parser.parse(hex_color)
    ...
    def test_color_accuracy_preservation(self, color_parser):
        ...
        for hex_color, expected_rgb in test_colors:
            parsed = color_parser.parse(hex_color)
            assert parsed is not None
            assert parsed.rgb_tuple == expected_rgb
    ...
    def test_batch_color_processing_performance(self, color_parser):
        ...
        parsed_colors = []
        for color_str in test_colors:
            parsed = color_parser.parse(color_str)
            if parsed:
...
class TestColorSystemAccuracyIntegration:
    ...
    def test_color_difference_calculations(self):
        ...
        green = parser.parse('#00FF00')
        blue = parser.parse('#0000FF')
        white = parser.parse('#FFFFFF')
        black = parser.parse('#000000')
        ...
        near_red = parser.parse('#FE0101')  # Very close to red
