...
def rgb_hex(c: Color|None) -> Optional[str]:
    if not c or c.value is None: return None
    r,g,b = int(c.red*255), int(c.green*255), int(c.blue*255)
    return f"{r:02X}{g:02X}{b:02X}"
def stroke_cap(v: Optional[str]) -> str:
    return {"butt":"flat","round":"rnd","square":"sq"}.get((v or "").lower(), "flat")
def stroke_join(v: Optional[str]) -> str:
    v = (v or "").lower()
...
# ---------------- Main ----------------
def convert(svg_path: str, minimal_txt: str, out_pptx: str) -> None:
